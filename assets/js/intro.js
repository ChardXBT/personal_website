import {
  ...
  REEL_ITEMS,
  STAT_KIND_LABELS
} from "../../content/site-content.js?v=20260922-2";
...
function createIntroExperience(doc = document, win = window) {
  ...
  function fillCollectionDetail() {
    const evidenceNodes = DATA_COLLECTION_EVIDENCE.map((item) => {
      ...
      description.textContent = STAT_KIND_LABELS[item.kind] ?? item.kind;
    ...
    const collectorNodes = DATA_COLLECTION_PROJECTS.map((collector) => {
      ...
      statKind.textContent = STAT_KIND_LABELS[collector.historicalStat.kind] ?? collector.historicalStat.kind;
