...
export function validateProjectContent() {
  ...
  assertContent(
    REEL_ITEMS.slice(0, -1).every((label, index) => (
      label === reelPhrase(PROJECTS[index])
    )),
...
function renderEvidenceStrip(lineEnding, indent) {
  return DATA_COLLECTION_EVIDENCE.map((item) => [
    ...
    `${indent}  <dd>${escapeHtml(STAT_KIND_LABELS[item.kind] ?? item.kind)}</dd>`,
    `${indent}</div>`
...
function renderCollectorList(lineEnding, indent) {
  return DATA_COLLECTION_PROJECTS.map((collector) => [
    ...
    `${indent}      <p class="collector-stat"><strong>${escapeHtml(collector.historicalStat.display)}</strong><span>${escapeHtml(STAT_KIND_LABELS[collector.historicalStat.kind] ?? collector.historicalStat.kind)}</span></p>`,
    `${indent}    </div>`,
...
export function renderDataCollectionSection(lineEnding = "\n") {
  return [
    ...
    '          <h3 class="section-title" id="data-collection-title" tabindex="-1">Monitoring &amp; Data Tools</h3>',
    '          <div class="data-collection-body">',
    '            <p class="data-collection-intro">Six small collectors that watch social feeds and marketplaces, clean up what they find, and flag anything worth a look.</p>',
...
export function renderNoscriptProjects(lineEnding = "\n") {
  ...
  return [
    ...
    '          <h2 id="noscript-projects-title">Things I’ve built</h2>',
    projects,
    "        </section>"
  ].join(lineEnding);
}
